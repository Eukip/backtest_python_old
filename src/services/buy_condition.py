...
class BuyCondition:
    ...
    async def calculate(self, main_time, candles, timeframe):
        ...
        dict = {
            ...
            '4hours': main_time - (main_time % 14400) - 14400,
    ...
    async def RSI(self, indicator_data, i, oversold, main_candle, candles, **kwargs):
    ...
    async def AROON(self, indicator_data, i, oversold, overbought, main_candle, candles, **kwargs):
        ...
        if previous[0] is None:
            return
        aroon = int(previous[0] >=overbought and previous[1] <= oversold )
        return aroon
